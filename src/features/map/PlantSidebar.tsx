...
import { processSyncQueue, selectAllPlants } from "../../store/plantsSlice";
...
export default function PlantSidebar({ plant, onClose }: PlantSidebarProps) {
  ...
  const [compareItem, setCompareItem] = useState<Plant | null>(null);

  const locationHistory = useMemo(() => {
    if (!plant) return [];

    const targetPoint = [plant.longitude, plant.latitude];
    ...
    <>
      ...
      {compareItem && (
        <CompareSlider
          ...
          beforeDate={compareItem.createdAt || ""}
          afterImage={plant.imageUrl}
          afterDate={plant.createdAt || ""}
